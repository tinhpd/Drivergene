...
public class ParallelLoyalPoint {
	...
	//	private final float EPS = 2 * 1e-7f;
		...
		public LoyalPoint(CyNetwork network) {
			...
			EPS = (nodeCount<=100)? 1E-5f: (nodeCount<= 200) ? 1E-4f: (nodeCount<= 300)? 1E-3f: 1E-2f; // 6- added by TranTD
			initialize();
		}
		...
		public Map<Integer, Float> compute(int leader) {
			MAX_ITERATION = (int)Math.pow(nodeCount*nEdge,1.0 / 1.001); // 5- add by TranTD
