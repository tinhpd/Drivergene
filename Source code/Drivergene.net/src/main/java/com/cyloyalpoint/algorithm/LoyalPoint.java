...
public class LoyalPoint {
	...
	private float EPS = 1e-40f;//1- edited by TranTD
	...
	private int nEdge=0;// 2- added by TranTD
	...
	public LoyalPoint(CyNetwork network) {
		...
		nEdge=network.getEdgeCount();// 3- added by TranTD
		EPS = (nodeList.size()<=100)? 1E-5f: (nodeList.size()<= 200) ? 1E-4f: (nodeList.size()<= 300)? 1E-3f: 1E-2f; // 4- added by TranTD
		initialize();
	}
	...
	public void initialize() {
		...
		E = 1.0f / maxOutDegMixing - 1e-45f;// 4- edited by TranTD
	...
	public Map<String, Float> computeCompetitive(String leader, String againstLeader) {
		int maxIterations = (int)Math.pow(nodeList.size()*nEdge,1.0 / 1.001); // 5- add by TranTD
